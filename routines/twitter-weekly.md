...
Step 1 — Gather context: Work in the shivdhwaj/social_media repository on its DEFAULT branch (clone if needed). Read notes/ideas.md if it exists — real material there beats anything invented. Read the newest file in blog/ — that essay is this week's theme. If blog/ is empty or its newest file is older than 14 days, do NOT stop or ask: pick this week's theme yourself (from notes/ideas.md if present, otherwise a fresh practical angle on AI-augmented engineering management) and continue with all remaining steps. List recent files in twitter/ and read the last 2 weeks' tweets so nothing repeats.
